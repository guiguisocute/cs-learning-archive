...
public class regex_test {
    public static void main(String[] args) {
        ...
        sc.close();
        String regex1 = "\\d+(\\.\\d+){1}"; //匹配操作数,java好恶心啊，所有转义字符还要多打一个反斜杠，不是纯纯恶心人
        /*仅对作业表示疑惑：
         * 什么叫10是不被查找的？？，why有这个限制？
         */
        String regex2 = "[+\\-*/()]"; //匹配运算符

        System.out.print("提取的操作数:");
        Matcher m1 = java.util.regex.Pattern.compile(regex1).matcher(in);      //前一个参数是说把regex1编译成一个待匹配文本的对象（不可变，变成类似游标的作用，用于传递给Matcher的构造方法），.matcher(str)就是把str给输进去，传给matcher构造对象的另一个参数
        ...
        */
        ...
        Matcher m2 = java.util.regex.Pattern.compile(regex2).matcher(in);
        while (m2.find()) {
